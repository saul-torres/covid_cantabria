...
# vaccine
region_vaccine_url <- "https://covid19can-data.firebaseio.com/saludcantabria/dosis.json"
region_vaccine_url <- URLencode(region_vaccine_url)
region_vaccine_json <- fromJSON(region_vaccine_url)
region_vaccine_values <- as.data.frame(split(region_vaccine_json$value, ceiling(seq_along(region_vaccine_json$value)/2)))
region_vaccine_values <- as.data.frame(t(region_vaccine_values))
region_vaccine_df <- as.data.frame(cbind(as.character(region_vaccine_json$dimension$Fecha$category$label),
                                      region_vaccine_values))
region_vaccine_df$date <- sort(as.Date(region_vaccine_df[,1], format = "%d-%m-%Y"))



# merge datasets
region_data_df <- Reduce(function(x, y) merge(x, y, by = "date", all = TRUE),
...
colnames(region_data_df) <- c("date", #common column
                              ...
                              "total_vacc_admin", "total_vacc_available") #region_vaccine_df dataset
...
# ocupación de camas
library(reshape)
beds_data <- melt(region_data_df[,c(1,28,29)], id="date")
...
# vacunacion
vacc_data <- melt(region_data_df[,c(1,22,23)], id="date")
vacc_data$variable <- factor(vacc_data$variable, levels = c("total_vacc_available", "total_vacc_admin"))

p_vacc <- ggplot(vacc_data, aes(x=date, y=value, group=variable, fill=variable)) + 
  geom_ribbon(aes(x=date, ymax=value), ymin=0) +
  geom_line(size=.5, colour = "#646464") + 
  scale_fill_manual(breaks = c("total_vacc_available", "total_vacc_admin"),
                    labels = c("Disponibles", "Administradas"),
                    values=c("#bdbdbd", "#de2d26")) +
  coord_cartesian(xlim=c(region_data_df$date[length(region_data_df$date)]-90,region_data_df$date[length(region_data_df$date)]+1)) +
  labs(subtitle = "Evolución de la vacunación",
       x = "Fecha",
       y = "Número de dosis",
       fill = "Dosis",
...
# combined plot
p_combine <- grid.arrange(p_incid, p_active, p_dailyd, p_beds, p_test, p_positiv, p_vacc, 
                          ncol = 1, heights = c(1.1, 1, 1, 1.2, 1, 1.05, 1.25))
...
p_vacc_indiv <- ggplot(vacc_data, aes(x=date, y=value, group=variable, fill=variable)) + 
  geom_ribbon(aes(x=date, ymax=value), ymin=0) +
  geom_line(size=.5, colour = "#646464") + 
  scale_fill_manual(breaks = c("total_vacc_available", "total_vacc_admin"),
                    labels = c("Disponibles", "Administradas"),
  ...
  coord_cartesian(xlim=c(region_data_df$date[length(region_data_df$date)]-90,region_data_df$date[length(region_data_df$date)]+1)) +
  labs(title = paste("EVOLUCIÓN COVID19 EN CANTABRIA (", fecha_dia_plot, ")", sep=""),
       subtitle = "Evolución de la vacunación",
       x = "Fecha",
       y = "Número de dosis",
       fill = "Dosis",
       caption = "Datos: ICANE. Gob. Cantabria. Elaboración: @covidcantabria") +  
...
ggsave("images/regional_positiv.png", p_pcr_indiv,
       width = 30, height = 8, units = "cm")
ggsave("images/regional_vaccines.png", p_vacc_indiv,
       width = 30, height = 8, units = "cm")
